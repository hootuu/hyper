...
func newEngine[T Matter](deal Deal[T], ord *Order[T], f *Factory[T]) *Engine[T] {
	...
	e.fsm = hfsm.NewMachine().
		AddTransition(Draft, SubmitEvent, e.onSubmit).
		AddTransition(Initial, ConsenseEvent, e.onAdvanceWrap(Consensus)).
		AddTransition(Initial, CancelEvent, e.onAdvanceWrap(Canceled)).
		AddTransition(Initial, TimeoutEvent, e.onAdvanceWrap(Timeout)).
		AddTransition(Initial, CompleteEvent, e.onAdvanceWrap(Completed)).
		AddTransition(Consensus, ExecuteEvent, e.onAdvanceWrap(Executing)).
		AddTransition(Executing, CompleteEvent, e.onAdvanceWrap(Completed))
...
func (e *Engine[T]) Submit(ctx context.Context) error {
	target, err := e.fsm.Handle(ctx, e.ord.Status, SubmitEvent)
	if err != nil {
		return err
	}
	e.ord.Status = target
...
func (e *Engine[T]) Complete(ctx context.Context) (err error) {
	fmt.Println("hiorder.Complete:::::::", e.ord.Status)
