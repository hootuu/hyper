...
const (
	ttOrderTimeout = "NINE_ROLE_ORDER_TIMEOUT"
)
...
func ttListenOrderTimeout(ctx context.Context, orderID hiorder.ID, timeout time.Duration) {
	...
	if orderID == 0 {
		hlog.TraceFix("roleord.order.ttListenOrderTimeout: orderID is 0", ctx, nil)
		return
	}
	hretry.Universal(func() error {
		err := hyperplt.Postman().Send(ctx, &ticktock.DelayJob{
			Type:      ttOrderTimeout,
			ID:        fmt.Sprintf("NINE_ROLE:ORDER:TIMEOUT:%d", orderID),
			Payload:   []byte(fmt.Sprintf("%d", orderID)),
			UniqueTTL: 0,
			Delay:     timeout,
		})
		if err != nil {
			hlog.TraceFix("send ticktock:NINE_ROLE_ORDER_TIMEOUT failed", ctx, err, zap.Uint64("orderID", orderID))
			return err
		}
		return err
	})
