...
const (
	ttOrderTimeout = "GROUP_BUY_ORDER_TIMEOUT"
)
...
func ttListenOrderTimeout(ctx context.Context, orderID hiorder.ID, timeout time.Duration) {
	...
	if orderID == 0 {
		hlog.TraceFix("groupbuy.order.ttListenOrderTimeout: orderID is 0", ctx, nil)
		return
	}
	hretry.Universal(func() error {
		err := hyperplt.Postman().Send(ctx, &ticktock.DelayJob{
			Type:      ttOrderTimeout,
			ID:        fmt.Sprintf("GROUP_BUY:ORDER:TIMEOUT:%d", orderID),
			Payload:   []byte(fmt.Sprintf("%d", orderID)),
			UniqueTTL: 0,
			Delay:     timeout,
		})
		if err != nil {
			hlog.TraceFix("send ticktock:GROUP_BUY_ORDER_TIMEOUT failed", ctx, err, zap.Uint64("orderID", orderID))
			return err
		}
		return err
	})
