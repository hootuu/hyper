...
const (
	ttOrderTimeout = "PDD_ORD_ORDER_TIMEOUT"
)
...
func ttListenOrderTimeout(ctx context.Context, orderID hiorder.ID, timeout time.Duration) {
	...
	if orderID == 0 {
		hlog.TraceFix("pddord.order.ttListenOrderTimeout: orderID is 0", ctx, nil)
		return
	}
	hretry.Universal(func() error {
		err := hyperplt.Postman().Send(ctx, &ticktock.DelayJob{
			Type:      ttOrderTimeout,
			ID:        fmt.Sprintf("PDD_ORD:ORDER:TIMEOUT:%d", orderID),
			Payload:   []byte(fmt.Sprintf("%d", orderID)),
			UniqueTTL: 0,
			Delay:     timeout,
		})
		if err != nil {
			hlog.TraceFix("send ticktock:PDD_ORD_ORDER_TIMEOUT failed", ctx, err, zap.Uint64("orderID", orderID))
			return err
		}
		return err
	})
